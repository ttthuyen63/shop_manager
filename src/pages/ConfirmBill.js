...
export default function ConfirmBill() {
  ...
  const [confirmBill, setconfirmBillState] = useState(null);
  ...
  const getConfirmBillApi = async () => {
    try {
      const res = await customAxios.get("/admin/bill/all?param=false");
      setconfirmBillState(res?.data);
    } catch (error) {
      console.log("Lỗi");
    }
  };
  console.log("suBill...", confirmBill);
